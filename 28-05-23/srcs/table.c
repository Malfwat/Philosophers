...
t_table	*create_table(int ac, char **av)
{
	...
	table->mutex_cutlery = ft_calloc(1, sizeof(pthread_mutex_t) \
	* (table->params.nb_of_philo + 1));
	if (!table->mutex_cutlery)
...
t_supervisor	*create_supervisor(t_table *table)
{
	...
	supervisor->philo_tab = ft_calloc \
	(table->params.nb_of_philo + 1, sizeof(t_philo));
	if (!supervisor->philo_tab)
		return (free_supervisor(supervisor), NULL);
	if (!fill_philo_tab(table, supervisor->philo_tab, \
	table->params.nb_of_philo))
		return (free_supervisor(supervisor), NULL);
	return (supervisor);
}
